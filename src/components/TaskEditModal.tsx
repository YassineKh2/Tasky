...
interface TaskEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: string, updates: Partial<TaskDefinition>) => void;
  task: TaskDefinition;
}
export function TaskEditModal({
  ...
  const [duration, setDuration] = useState(task.baselineDuration);
  const [isRecurring, setIsRecurring] = useState(task.isRecurring);
  const [recurringDays, setRecurringDays] = useState<number[]>(
    task.recurringDays,
  );
  const [startDate, setStartDate] = useState(task.startDate || getLocalDateStr(new Date()));
  const [endDate, setEndDate] = useState<string>(task.endDate || "");
  const [hasEndDate, setHasEndDate] = useState(!!task.endDate);
  const weekDays = [
    {
      label: "S",
      value: 0,
  ...
  const handleSave = () => {
    ...
    onSave(task.id, {
      ...
      startDate: isRecurring ? startDate : task.startDate,
